...
var nepesseg = parseInt($('#nepesseg').text());
var korenkent = Math.floor(nepesseg * 0.1);
var naponta = korenkent * 24; // Feltételezve, hogy 24 kör van naponta
$('#nepesseg').html($('#nepesseg').html() + '<br>Képezhető katonák körönként: ' + korenkent + '<br>Képezhető katonák naponta: ' + naponta);
...
// 1. Új függvények (a fájl végére)
var szamolHazak = function() {
    var terulet = parseInt($('#hektar').val());
    var lakashelyzeti = parseFloat($('#lakashelyzet').val()) / 100;
    var optimalisHazak = Math.floor((terulet * 15) / (50 * (1 + lakashelyzeti) + 15));
    $('#haz').val(optimalisHazak);
    szamol();
};